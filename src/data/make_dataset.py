...
def main(config):
    ...
    training_images = images_to_tensor(
        os.path.join(root, hparams["path_extract"], hparams["raw_dataset"], hparams["dataset_name"]),
        transform=transform,
    )
    training_labels = labels_to_tensor(
        os.path.join(root, hparams["path_extract"], hparams["raw_dataset"]), dataset_name=hparams["dataset_name"]
    )
